...
import { useSessionsStore, useSettingsStore, useToastStore } from '../store';
import ConfirmModal from '../components/ConfirmModal';

export default function SessionsScreen() {
  const { sessions, setSessions, addSession, deleteSession, setActiveSession } = useSessionsStore();
  ...
  const loadSessions = async () => {
    try {
      ...
      addToast(language === 'ar' ? 'فشل تحميل الجلسات' : 'Failed to load sessions', 'error');
    }
  };
  ...
  const handleNewSession = async () => {
    ...
    try {
      ...
      addSession(session);
      setActiveSession(id);
      addToast(language === 'ar' ? 'تم إنشاء جلسة جديدة' : 'New session created', 'success');
    } catch (err: any) {
      console.error('Failed to create session:', err);
      addToast(language === 'ar' ? 'فشل إنشاء الجلسة' : 'Failed to create session', 'error');
    }
  };
  ...
  const confirmDelete = async () => {
    if (deleteId) {
      try {
        await window.api?.deleteSession?.(deleteId);
        deleteSession(deleteId);
        setDeleteId(null);
        addToast(language === 'ar' ? 'تم حذف الجلسة' : 'Session deleted', 'info');
      } catch (err: any) {
        console.error('Failed to delete session:', err);
        addToast(language === 'ar' ? 'فشل حذف الجلسة' : 'Failed to delete session', 'error');
      }
    }
  };
  ...
  const handleExport = () => {
    try {
      ...
      addToast(language === 'ar' ? 'تم تصدير الجلسات' : 'Sessions exported', 'success');
    } catch (err) {
        ...
        try {
          ...
          if (Array.isArray(imported)) {
             for (const s of imported) {
               ...
               const newSession = { ...s, id: newId };
               await window.api?.saveSession?.(newSession);
               addSession(newSession);
             }
             addToast(language === 'ar' ? 'تم استيراد الجلسات بنجاح' : 'Sessions imported successfully', 'success');
          }
        } catch (err) {
          console.error("Failed to import sessions", err);
          addToast(language === 'ar' ? 'فشل استيراد الجلسات' : 'Failed to import sessions', 'error');
        }
      };
