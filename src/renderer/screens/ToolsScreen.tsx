...
import { Wrench, ToggleLeft, ToggleRight, Info, Search, Terminal, FileText, Globe, Eye, Code, Image, Volume2 } from 'lucide-react';
...
const iconMap: Record<string, any> = {
  Search, Terminal, FileText, Globe, Eye, Code, Image, Volume2, Wrench
};
...
export default function ToolsScreen() {
  ...
  const loadTools = async () => {
    try {
      ...
      addToast(language === 'ar' ? 'فشل تحميل الأدوات' : 'Failed to load tools', 'error');
    }
  };
  ...
  const toggleTool = async (toolId: string, enabled: boolean) => {
    try {
      ...
      addToast(`${enabled ? (language === 'ar' ? 'تم تفعيل' : 'Enabled') : (language === 'ar' ? 'تم تعطيل' : 'Disabled')} ${toolId}`, 'success');
      loadTools();
    } catch (err: any) {
      console.error('Failed to toggle tool:', err);
      addToast(language === 'ar' ? 'فشل تغيير حالة الأداة' : 'Failed to toggle tool state', 'error');
    }
  };
    ...
    <div className="flex flex-col h-full bg-[#121212] p-8">
      ...
      {tools.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {tools.map(tool => {
            const IconComponent = iconMap[tool.icon || 'Wrench'] || Wrench;
            return (
              <div key={tool.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex flex-col hover:border-gray-700 transition-colors">
                 <div className="flex justify-between items-start mb-4">
                   <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${tool.enabled ? 'bg-blue-500/10 text-blue-400' : 'bg-gray-800 text-gray-500'}`}>
