...
ipcMain.handle('zoom:get-profiles', async () => {
  return await getProfilesInternal();
});
...
ipcMain.handle('zoom:toggle-tool', async (event, toolId: string, enabled: boolean) => {
  ...
  await writeConfigInternal(config);
  return true;
});
...
async function readConfigInternal() {
  const defaultTools = [
    { id: 'web-search', name: 'Web Search', description: 'Search the internet for information', enabled: true, icon: 'Search' },
    { id: 'terminal', name: 'Terminal', description: 'Execute shell commands', enabled: false, icon: 'Terminal' },
    { id: 'file-ops', name: 'File Operations', description: 'Read and write files', enabled: true, icon: 'FileText' },
    { id: 'browser', name: 'Browser', description: 'Browse websites and interact with them', enabled: false, icon: 'Globe' },
    { id: 'vision', name: 'Vision', description: 'Analyze images and screenshots', enabled: false, icon: 'Eye' },
    { id: 'code-exec', name: 'Code Execution', description: 'Run Python code safely', enabled: true, icon: 'Code' },
    { id: 'image-gen', name: 'Image Generation', description: 'Generate images with AI', enabled: false, icon: 'Image' },
    { id: 'tts', name: 'Text to Speech', description: 'Convert text to audio', enabled: false, icon: 'Volume2' },
  ];
  ...
  // Ensure tools exist even in existing configs
...
ipcMain.handle('config:read', async () => {
  return await readConfigInternal();
});

/** Write config */
async function writeConfigInternal(config: any) {
  ensureZoomDirs();
...
ipcMain.handle('config:write', async (event, config: any) => {
  return await writeConfigInternal(config);
});
