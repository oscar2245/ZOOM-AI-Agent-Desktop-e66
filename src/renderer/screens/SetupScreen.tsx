...
export default function SetupScreen() {
  ...
  useEffect(() => {
    ...
    const removeProgress = window.api?.onInstallProgress?.((_e, msg) => {
      setInstallLog(prev => [...prev, msg]);
      if (msg.toLowerCase().includes('done') || msg.toLowerCase().includes('complete') || msg.toLowerCase().includes('success')) {
        setIsInstalling(false);
        setStep(2);
      }
    });
    
    const removeError = window.api?.onInstallError?.((_e, msg) => {
      setInstallLog(prev => [...prev, `ERROR: ${msg}`]);
      setIsInstalling(false);
      addToast(`Installation error: ${msg}`, 'error');
    });
    
    const removeComplete = window.api?.onInstallComplete?.((_e, success) => {
      setIsInstalling(false);
      if (success) {
        setStep(2);
        addToast('Installation successful!', 'success');
      } else {
  ...
  const startInstall = async () => {
    ...
    setInstallLog(['Initializing installer...']);
    try {
  ...
  const handleFinish = async () => {
    try {
      await window.api?.setProvider('OpenRouter', 'demo-key');
      addToast('Setup completed!', 'success');
      navigate('/chat');
    } catch (err: any) {
      console.error('Failed to finish setup:', err);
      addToast(`Failed to finish setup: ${err.message}`, 'error');
    }
  };
